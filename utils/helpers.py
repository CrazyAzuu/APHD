...
def extract(a, t, x_shape):
    b, l, *_ = x_shape
    # repeat t to match a's shape
    t = torch.repeat_interleave(t, l, dim=0).reshape(-1, l)
    out = a.gather(0, t)
    return out.reshape(b, l, *((1,) * (len(x_shape) - 2)))

# def extract_v2(a, t, x_shape):
#     b, *_ = t.shape
#     out = a.gather(-1, t)
...
def linear_beta_schedule(timesteps, beta_start=1e-3, beta_end=2e-2, dtype=torch.float32):
    betas = np.linspace(
